...
export function ListOf<TItem>(props: ListPropsOf<TItem>): React.ReactNode {
	...
	return (
		<Layout {...props}>
			...
			{Nullable.Value(props.showMoreLimit, <></>, (limit) => !showMore && props.items.length > limit + 1 && <Button {...props.showMoreButtonStyles} type="button" onClick={() => { setShowMore(true); }}>+{props.items.length - limit} <TranslatedText textKey="ButtonMore" /></Button>)}
		</Layout>
	);
}
