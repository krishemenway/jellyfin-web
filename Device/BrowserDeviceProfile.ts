import { CodecProfile, DeviceProfile, DirectPlayProfile, ProfileCondition, TranscodingProfile } from "@jellyfin/sdk/lib/generated-client/models";
import { Browser } from "Device/Browser";

const userSettings = {
	preferFmp4HlsContainer: () => false,
...
const audioTestElement: HTMLAudioElement = document.createElement('audio');
const videoTestElement: HTMLVideoElement = document.createElement("video");
const canvasTestElement: HTMLCanvasElement = document.createElement('canvas');

export interface DeviceProfileSetupOptions {
	audioChannels: number|undefined;

	enableHls: boolean|undefined;
	enablePgsRender: boolean|undefined;
	enableSsaRender: boolean|undefined;

	supportsHevc: boolean|undefined;
	supportsMp2VideoAudio: boolean|null;
	supportsTrueHd: boolean|null;
	...
	disableVideoAudioCodecs: string[]|null;
	disableHlsVideoAudioCodecs: string[]|null;
}
...
function canPlayHevc(options: DeviceProfileSetupOptions) {
...
let _supportsTextTracks: boolean|null;
function supportsTextTracks() {
	...
	if (_supportsTextTracks == null) {
		_supportsTextTracks = videoTestElement.textTracks != null;
	}
...
let _supportsCanvas2D: boolean|null;
function supportsCanvas2D() {
	if (_supportsCanvas2D == null) {
		_supportsCanvas2D = canvasTestElement.getContext('2d') != null;
	}

	return _supportsCanvas2D;
}

let _canPlayHls: boolean|null;
function canPlayHls() {
	if (_canPlayHls == null) {
...
function canPlayNativeHls(): boolean {
	...
	return !!(videoTestElement.canPlayType('application/x-mpegURL').replace(/no/, '')
 ...
 */
function canPlayDts(): boolean|null {
...
function supportsAc3InHls(): boolean {
	if (Browser.tizen || Browser.web0s) {
		return true;
	}
...
function supportsMp3InHls(): boolean {
	if (videoTestElement.canPlayType) {
...
function canPlayAudioFormat(format: string): boolean {
	...
	return !!audioTestElement.canPlayType(typeString).replace(/no/, '');
}

function testCanPlayMkv(): boolean {
	if (Browser.vidaa) {
		return false;
	}
...
function supportsHdr10(options: DeviceProfileSetupOptions): boolean {
	return options.supportsHdr10 ?? (
			...
			|| (Browser.firefox && Browser.osx && (!Browser.iphone && !Browser.ipad) && (Browser.versionMajor >= 100))
	);
}

function supportsHlg(options: DeviceProfileSetupOptions): boolean {
	return options.supportsHlg ?? supportsHdr10(options);
}

function supportsDolbyVision(options: DeviceProfileSetupOptions): boolean {
	return options.supportsDolbyVision ?? (Browser.safari && ((Browser.iOS && Browser.iOSVersion >= 13) || Browser.osx));
}
...
function getDirectPlayProfileForVideoContainer(containerType: string, videoAudioCodecs: string[], options: DeviceProfileSetupOptions): DirectPlayProfile|null {
	let supported = false;
	let profileContainer = containerType;
	const videoCodecs = [];
	...
	switch (containerType) {
		...
		case 'ts':
			...
			if ((Browser.tizen || Browser.web0s) && canPlayHevc(options)) {
				videoCodecs.push('hevc');
			}
			if (supportsVc1()) {
				videoCodecs.push('vc1');
...
function getSpeakerCount(): number {
	...
	const AudioContext = window.AudioContext || "webkitAudioContext" in window;
...
function getPhysicalAudioChannels(options: DeviceProfileSetupOptions): number {
	...
	const isAc3Eac3Supported = supportsAc3() || supportsEac3();
...
export default function (options?: DeviceProfileSetupOptions): DeviceProfile {
	options = options || { } as DeviceProfileSetupOptions;
	...
	const safariSupportsOpus = Browser.safari && Browser.versionMajor >= 17 && !!audioTestElement.canPlayType('audio/x-caf; codecs="opus"').replace(/no/, '');
	const webmAudioCodecs = ['vorbis'];

	const canPlayMkv = testCanPlayMkv();
	...
	const canPlayMp3VideoAudioInHls = supportsMp3InHls();
	const canPlayAc3VideoAudio = supportsAc3();
	const canPlayEac3VideoAudio = supportsEac3();
	const canPlayAc3VideoAudioInHls = supportsAc3InHls();
	...
	['m2ts', 'wmv', 'ts', 'asf', 'avi', 'mpg', 'mpeg', 'flv', '3gp', 'mts', 'trp', 'vob', 'vro', 'mov'].map(function (containerType) {
		return getDirectPlayProfileForVideoContainer(containerType, videoAudioCodecs, options);
	}).filter((p) => p != null).forEach((p) => profile.DirectPlayProfiles?.push(p));
	...
	const supportsSecondaryAudio = canPlaySecondaryAudio();

	const aacCodecProfileConditions: ProfileCondition[] = [];
	...
	const globalAudioCodecProfileConditions: ProfileCondition[] = [];
	const globalVideoAudioCodecProfileConditions: ProfileCondition[] = [];
	...
	if (Browser.web0s) {
		const flacConditions: ProfileCondition[] = [
		...
		profile.TranscodingProfiles.forEach(transcodingProfile => {
			...
			const flacTranscodingProfile = { ...transcodingProfile } as TranscodingProfile;
			flacTranscodingProfile.AudioCodec = 'flac';
			flacTranscodingProfile.Conditions = [
				...flacTranscodingProfile.Conditions || [],
				...flacConditions
			];
	...
	if (safariSupportsOpus) {
		const opusConditions: ProfileCondition[] = [
		...
		profile.TranscodingProfiles.forEach(transcodingProfile => {
			...
			opusTranscodingProfile.Conditions = [
				...opusTranscodingProfile.Conditions || [],
				...opusConditions
			];
	...
	if ((Browser.tizenVersion >= 5 && "NativeShell" in window)
	...
	if (supportsDolbyVision(options)) {
		...
		if (supportedDolbyVisionProfileAv1()) {
	...
	const vp9CodecProfileConditions: ProfileCondition[] = [
		{
			Condition: 'EqualsAny',
			Property: 'VideoRangeType',
	...
	const av1CodecProfileConditions: ProfileCondition[] = [
		{
			Condition: 'NotEquals',
			Property: 'IsAnamorphic',
			Value: 'true',
	...
	const globalVideoConditions: ProfileCondition[] = [];
