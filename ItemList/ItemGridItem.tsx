...
export const ItemsGridItem: React.FC<{ item: BaseItemDto; library: BaseItemDto, imageType?: ImageType; shape: ImageShape; itemsPerRow: number }> = (props) => {
	...
	return (
		<LinkToItem item={props.item} className={`${background.button}`} direction="column" justifyContent="space-between" alignItems="center" py=".5em" px=".5em" gap="1em" width={{ itemsPerRow: props.itemsPerRow, gap: ".5em" }}>
			<ItemImage item={props.item} fallback={props.library} className={classes.itemImage} type={props.imageType ?? ImageType.Primary} fillWidth={width} fillHeight={height} lazy objectFit="cover" grow />
